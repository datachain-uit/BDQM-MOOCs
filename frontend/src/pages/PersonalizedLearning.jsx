...
const MemoizedBar = React.memo(({ data, options }) => (
...
const PersonalizedLearning = () => {
    ...
    const fetchData = async () => {
      try {
        const response = await fetch(
          "http://localhost:8000/api/recommender_datainfo/"
        );
        ...
        setRelevance5Precision(
          result.data[0].evaluation.relevanceData.at5.precision
        );
        setRelevance10Precision(
          result.data[0].evaluation.relevanceData.at10.precision
        );
        setRelevance5Recall(result.data[0].evaluation.relevanceData.at5.recall);
        setRelevance10Recall(
          result.data[0].evaluation.relevanceData.at10.recall
        );
        ...
        setConsistencyPassRate(
          result.data[0].evaluation.consistencyData.overallPassRate
        ); // Assuming the API returns an array of objects
        setCompletenessPassRate(
          result.data[0].evaluation.completenessData.overallPaseRate
        ); // Assuming the API returns an array of objects
        setDescriptiveStatistics1(
          result.data[0].descriptiveStatistics.enroll_time
        );
        setDescriptiveStatistics2(
          result.data[0].descriptiveStatistics.user_gender
        );
        setDescriptiveStatistics3(
          result.data[0].descriptiveStatistics.course_total_comments
        );
        setDescriptiveStatistics4(
          result.data[0].descriptiveStatistics.user_course_num_comment
        );
        setDescriptiveStatistics5(
          result.data[0].descriptiveStatistics.user_course_num_replies
        );
        console.log("API result:", result);
  ...
  const reliabilityData = {
    "@5": {
      labels: models,
      datasets: [
        { label: "MAP", data: reliability5map, backgroundColor: "#0071BC" },
      ],
    },
    "@10": {
      labels: models,
      datasets: [
        { label: "MAP", data: reliability10map, backgroundColor: "#0071BC" },
      ],
    },
  };
  ...
  const relevanceData = {
    ...
    "@10": {
      ...
      datasets: [
        {
          label: "Precision",
          data: relevance10precision,
          backgroundColor: "#0071BC",
        },
        {
          label: "Recall",
          data: relevance10recall,
    ...
    definition: {
      ...
      text: {
        completeness: {
          term: "Completeness:",
          definition:
            "The dataset must contain all necessary information without missing any critical elements.",
        },
        approach: {
          term: "Measurement approach:",
          definition:
            "Completeness is calculated as the ratio between the number of valid (non-missing) values and the total number of expected values.",
        },
      },
    },
    errorLog: [
  ...
  const consistencyData = {
    ...
    definition: {
      ...
      text: {
        intro:
          "<strong style='font-weight: 600;'>Consistency:</strong> Data must be uniform across different sources and systems, with no conflicts or duplications.",
        heading: "Basic Validity Checks:",
        criteria: [
          {
            name: "Domain Range",
            description: "The value must fall within a predefined range.",
          },
          {
            name: "Non-null",
            description: "The value must not be empty or missing.",
          },
          {
            name: "Data Type",
            description: "The value must conform to a specified data type.",
          },
          {
            name: "Logical Constraints",
            description: "The value must satisfy a logical condition.",
          },
          {
            name: "Uniqueness",
            description: "The value must be unique within the dataset.",
          },
          {
            name: "Foreign Key Integrity",
            description: "The value must exist in a valid reference list.",
          },
        ],
      },
    },
  ...
  const currentData =
    selectedMetric === "completeness" ? completenessData : consistencyData;
  ...
  return (
    ...
    >
      ...
      <CCard style={commonStyles.card}>
        <CCardHeader style={commonStyles.textBold}>
          Dataset Information
        </CCardHeader>
        <CCardBody style={{ padding: 12, paddingTop: 0, paddingBottom: 0 }}>
          ...
          <CTable responsive style={commonStyles.table}>
            <CTableBody>
              {Object.keys(keyMap).map((key) => (
                <CTableRow key={key}>
                  <CTableHeaderCell scope="row" style={commonStyles.textBold}>
                    {key}:
                  </CTableHeaderCell>
                  <CTableDataCell className="text-end">
                    {datasetInfo[keyMap[key]]}
                  </CTableDataCell>
                </CTableRow>
              ))}
            </CTableBody>
          ...
          <CTable striped bordered responsive style={commonStyles.table}>
            <CTableHead>
              <CTableRow>
                <CTableHeaderCell scope="col" style={commonStyles.textBold}>
                  Statistic
                </CTableHeaderCell>
      ...
      >
        ...
        >
          {[
            ...
            },
            {
              ...
              options: {
                ...
                plugins: {
                  ...
                  tooltip: {
                    callbacks: { label: (ctx) => `${ctx.parsed.y} learners` },
                  },
                },
            ...
            >
              <h6
                style={{ marginBottom: 4, color: "#000", textAlign: "center" }}
              >
                {title}
              </h6>
        ...
        <CCol xs={6} style={{ paddingRight: 4 }}>
          ...
          >
            ...
            <CCardBody style={{ padding: "20px 20px 15px" }}>
                  ...
                  <>
                    ...
                    >
                      <strong style={{ fontWeight: 600 }}>
                        {currentData.definition.text.completeness.term}
                      </strong>{" "}
                      {currentData.definition.text.completeness.definition}
                    </p>
                    <p
                      style={{
                        fontSize: 18,
                    ...
                    >
                      <strong style={{ fontWeight: 600 }}>
                        {currentData.definition.text.approach.term}
                      </strong>{" "}
                      {currentData.definition.text.approach.definition}
                    </p>
                  </>
                ) : (
                  <div>
                    <p
                      ...
                      dangerouslySetInnerHTML={{
                        __html: currentData.definition.text.intro,
                      }}
                    />
                    <p
                      style={{
              ...
              <div
                style={{ borderTop: "2px dashed #828282", margin: "20px 0" }}
              />
              ...
              {[
                {
                  ...
                  setMetric: setReliabilityMetric,
                  data: reliabilityData,
                },
                {
                  label: "Relevance",
                  metric: relevanceMetric,
                  setMetric: setRelevanceMetric,
                  data: relevanceData,
                },
